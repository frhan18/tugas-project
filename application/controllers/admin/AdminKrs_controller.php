...
class AdminKrs_controller extends CI_Controller
{
    ...
    public function index() // Dashboard
    {
        ...
        $data['krs'] = $this->db->select('*, tb_mahasiswa.nim')
            ->from('tb_krs')
            ...
            ->join('tb_mahasiswa', 'tb_mahasiswa.kode_kelas=tb_krs.kode_kelas')
            ->get()->result_array();
        ...
        $data['count_tb_krs'] = $this->db->count_all('tb_krs');
        $config = [
    ...
    private function add_krs()
    {
        ...
        if ($this->db->insert('tb_krs', $data)) {
            $this->session->set_flashdata('message_success', ' Data krs ditambahkan');
            redirect('data-krs');
        }
    }
    ...
    public function delete_krs($id)
    {
        ...
        if (!$row['id'] || !$id) {
            ...
            $this->session->set_flashdata('message_success', 'Data krs dihapus');
            redirect('data-krs');
        }
    }
    ...
    public function update_krs($id)
    {
        ...
        if (!$row['id'] || !$id) {
            ...
            if ($this->db->update('tb_krs', $data, ['id' => $id])) {
                $this->session->set_flashdata('message_success', 'Data krs diperbarui');
                redirect('data-krs');
            }
        }
    }
